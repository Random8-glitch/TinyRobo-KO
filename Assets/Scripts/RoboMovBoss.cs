...
public class RoboMovBoss : MonoBehaviour
{
    ...
    [Header("Boost por vida")]
    [Tooltip("Porcentaje de incremento aplicado a las estadísticas de movimiento cuando la vida baja del 50%.")]
    [SerializeField] private float movementBoostPercent = 10f;
    ...
    private bool movementBoostApplied = false;
    ...
    private void Start()
    {
        ...
        // Obtener componente EnemyStats si existe
        enemyStats = GetComponent<EnemyStats>();
    }

    private void Update()
    {
        // Chequear vida y aplicar boost si corresponde (se aplica una sola vez)
        if (!movementBoostApplied && enemyStats != null)
        {
            if (enemyStats.vida <= enemyStats.vidaMax * 0.5f)
            {
                ApplyMovementBoost();
            }
        }
    ...
    {
        movementBoostApplied = true;

        float factor = 1f + (movementBoostPercent / 100f);

        velocidad *= factor;
        velocidadGiroMinima *= factor;
        velocidadGiroMaxima *= factor;
        velocidadGiroActual *= factor;

        Debug.Log($"RoboMovBoss: vida < 50% — aplicado movement boost {movementBoostPercent}% (factor {factor:F2}).", this);
    }
